...
option_list = list(
  ...
  make_option(c("-d", "--medoid"), type="character", default = "yes",
              help="output medoid information for a given mode [default= %default]. \n \t Available methods are: 'ward.D', 'ward.D2', 'single', 'complete', 'average', 'mcquitty', 'median', or 'centroid'", metavar = 'charcter')
); 
...
if (opt$mode == 'heatmap'){
  ...
  if (opt$medoid == 'yes'){
    medoid_centers <- as.matrix(aggregate(mash_df, list(mcl_mash), mean))
    outfile <- paste(strsplit(opt$file, '_distmat.csv')[[1]][1],'_r',out_prefix,'_medoid_centers.csv', sep = '')
    write.matrix(medoid_centers, file=outfile, sep=',')
  }
}
